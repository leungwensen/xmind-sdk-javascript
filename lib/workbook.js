...
class Workbook extends DomMixin {
  ...
  _loadSheets() {
    const me = this;
    utils.eachChildNode(me.doc, {
      tagName: CONST.TAG_SHEET,
    }, (childNode) => {
  ...
  removeSheet(sheet/* index or id or instance */) {
    const me = this;
    if (me.sheets.length <= 1) return me; // primary sheet cannot be removed
    if (lang.isNumber(sheet)) {
      sheet = me.sheets[sheet];
    } else if (lang.isString(sheet)) {
      sheet = me.sheetById[sheet];
    }
    const index = lang.indexOf(me.sheets, sheet);
    const id = sheet.id;
...
lang.extend(Workbook, {
  load(data/* zip packed data */) {
    ...
    if (!doc) throw new Error('invalid xmind file');
