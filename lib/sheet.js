...
class Sheet extends DomMixin {
  ...
  _loadLegend() {
    const me = this;
    const legenNode = utils.findChildNodes(me.doc, {
      tagName: CONST.TAG_LEGEND,
    })[0];
    if (legenNode) {
      new Legend({
        sheet: me,
  ...
  _loadRootTopic() {
    const me = this;
    const rootTopicNode = utils.findChildNode(me.doc, {
  ...
  _loadRelationships() {
    const me = this;
    const relationshipsNode = utils.findChildNodes(me.doc, {
  ...
  addRelationship(options) {
    const me = this;
    me.relationshipsNode = utils.findOrCreateChildNode(me.doc, {
      tagName: CONST.TAG_RELATIONSHIPS,
    });
    me.setModifiedTime();
