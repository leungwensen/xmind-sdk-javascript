...
class Relationship extends DomMixin {
  constructor(options) {
    ...
    const relationshipsNode = utils.findOrCreateChildNode(sheet.doc, {
      tagName: CONST.TAG_RELATIONSHIPS,
    });
    ...
    if (options.doc) {
      ...
      if (utils.findChildNode(relationshipsNode, {
          tagName: CONST.TAG_RELATIONSHIP,
          attributes: attrs,
        })) {
        throw new Error('the same relationship already exists');
      }
  ...
  setSource(value) {
    ...
    if (targetId === value) throw new Error('source & target should not be the same');
    if (value) {
  ...
  setTarget(value) {
    ...
    if (sourceId === value) throw new Error('source & target should not be the same');
    if (value) {
