...
class DomMixin {
  ...
  // child node
  eachChildNode(query, callback) {
    utils.eachChildNode(this.doc, query, callback);
  }

  findOrCreateChildNode(query) {
    return utils.findOrCreateChildNode(this.doc, query);
  }
  ...
  // title
  getTitle() {
    const titleNode = utils.findChildNode(this.doc, {
      tagName: CONST.TAG_TITLE,
    });
    return titleNode ? titleNode.textContent : '';
  }

  setTitle(title) {
    const titleNode = utils.findOrCreateChildNode(this.doc, {
      tagName: CONST.TAG_TITLE,
    });
  ...
  // position
  getPosition() {
    const positionNode = utils.findChildNode(this.doc, {
      tagName: CONST.TAG_POSITION,
    });
    return positionNode ? {
  ...
  setPosition(position) {
    const positionNode = utils.findOrCreateChildNode(this.doc, {
      tagName: CONST.TAG_POSITION,
    });
  ...
  removePosition() {
    utils.removeChildNode(this.doc, {
      tagName: CONST.TAG_POSITION,
    });
    return this;
  }
