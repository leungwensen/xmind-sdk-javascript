...
class Topic extends DomMixin {
  ...
  _addToParent(parent) {
    ...
    if (me.isRootTopic) {
      ...
      const childrenNode = utils.findOrCreateChildNode(parentDoc, {
  ...
  _loadSubTopics() {
     ...
     */
    const me = this;
    const childrenNodes = utils.findChildNodes(me.doc, {
      tagName: CONST.TAG_CHILDREN,
    });
    lang.each(childrenNodes, (childrenNode) => {
      const topicsNodes = utils.findChildNodes(childrenNode, {
        tagName: CONST.TAG_TOPICS,
      });
      lang.each(topicsNodes, (topicsNode) => {
        const type = topicsNode.getAttribute(CONST.ATTR_TYPE);
        const topicNodes = utils.findChildNodes(topicsNode, {
  ...
  // notes
  getNotes() {
    const notesNode = utils.findChildNode(this.doc, {
      tagName: CONST.TAG_NOTES,
    });
    if (notesNode) {
      const plainNotesNode = utils.findChildNode(notesNode, {
        tagName: CONST.PLAIN_FORMAT_NOTE,
      });
  ...
  // labels
  getLabels() {
    const labelsNode = utils.findChildNode(this.doc, {
      tagName: CONST.TAG_LABELS,
    });
    if (!labelsNode) return [];
    return lang.map(utils.findChildNodes(labelsNode, {
      tagName: CONST.TAG_LABEL,
  ...
  getMarkers() {
    const result = [];
    const markersNode = utils.findChildNode(this.doc, {
      tagName: CONST.TAG_MARKERREFS,
    });
    if (markersNode) {
      utils.eachChildNode(markersNode, {
        tagName: CONST.TAG_MARKERREF,
      }, (node) => {
  ...
  addMarker(id) {
    ...
    const me = this;
    const markersNode = utils.findOrCreateChildNode(me.doc, {
      tagName: CONST.TAG_MARKERREFS,
    });
    const attrs = {};
    attrs[CONST.ATTR_MARKERID] = id;
    if (utils.findChildNode(me.doc, {
        tagName: CONST.TAG_MARKERREF,
        attributes: attrs,
  ...
  removeMarker(id) {
    ...
    const attrs = {};
    attrs[CONST.ATTR_MARKERID] = id;
    utils.removeChildNode(markersNode, {
      tagName: CONST.TAG_MARKERREF,
      attributes: attrs,
    });
    return me.setModifiedTime();
