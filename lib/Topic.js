...
class Topic extends DomMixin {
  ...
  moveTo(targetTopic) {
    const me = this;
    targetTopic = Topic.getTopic(targetTopic, me.sheet);
    if (!targetTopic) throw 'target topic does not exist';
    if (me.isRootTopic) throw 'cannot move root topic';
    if (me === targetTopic) throw 'cannot move to itself';
    if (me.isAncestorOf(targetTopic)) throw 'cannot move to a child topic';
  ...
  addMarker(id) {
    ...
    if (utils.findChildNode(me.doc, CONST.TAG_MARKERREF, attrs)) throw 'marker already exists!';
